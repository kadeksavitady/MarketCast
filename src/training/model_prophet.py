...
==============================
Baseline 2: Facebook Prophet — Hybrid Expanding+Sliding CV
────────────────────────────────────────────────────────────
...
DUA MODE EKSEKUSI (dikontrol dari train_all.py):
    mode="tournament"
        Tujuan    : bandingkan Prophet vs SARIMA vs XGBoost
        CV        : Hybrid Expanding+Sliding, 5 split (fallback 3)
        Fitting   : Prophet dengan changepoint_scale auto per cluster
        Hyperparams: changepoint_prior_scale & seasonality_prior_scale default
        Output    : weighted MAPE sebagai metrik perbandingan di leaderboard

    mode="specialize"
        Tujuan    : training final 43 komoditas dengan Prophet sebagai champion
        CV        : Hybrid Expanding+Sliding, 5 split (fallback 3)
        Tuning    : Optuna TPE pada split terakhir (paling representatif)
                    n_trials=15, early stopping jika 5 trial tidak improve
                    Parameter yang di-tune:
                        changepoint_prior_scale : 0.001 – 0.5  (log-uniform)
                        seasonality_prior_scale : 1.0   – 50.0 (log-uniform)
                        fourier_order_monthly   : 3     – 10   (int)
                    Setelah best params ditemukan → refit di full series

KENAPA OPTUNA UNTUK PROPHET (bukan GridSearch/Random):
    - changepoint_prior_scale dan seasonality_prior_scale adalah continuous
      parameter → grid search butuh titik manual yang kasar
    - Optuna TPE (Tree-structured Parzen Estimator) belajar dari tiap trial
      → konvergen lebih cepat ke area optimal
    - n_trials=15 dipilih karena Prophet lambat (~10-30s/trial via Stan)
      43 komoditas × 15 trial = 645 runs, masih feasible untuk 1 semester
    - Early stopping (patience=5) mengurangi waktu untuk komoditas
      yang params-nya tidak sensitif (Cluster 2: stabil, cepat konvergen)

HYBRID EXPANDING + SLIDING CV (split scheme):
    Data: Mei 2021 → Mei 2026 (1827 hari)
    Min train: 1095 hari (3 tahun)
    Test window: 30 hari

    Split 1: Train [Mei2021─Apr2024] (1095d) → Test [Mei2024]    expanding
    Split 2: Train [Mei2021─Jun2024] (1125d) → Test [Jul2024]    expanding
    Split 3: Train [Mei2021─Agu2024] (1155d) → Test [Sep2024]    expanding
    Split 4: Train [Jul2021─Okt2024] (1185d) → Test [Nov2024]    sliding
    Split 5: Train [Sep2021─Des2024] (1185d) → Test [Jan2025]    sliding

    Weighted MAPE = Σ(w_i × MAPE_i) dengan bobot [0.10, 0.15, 0.20, 0.25, 0.30]
    Split terbaru (paling dekat ke masa depan) mendapat bobot lebih besar.

FALLBACK DATA PENDEK:
    Jika total data < min_train + test_window × n_splits:
    → Turunkan ke 3 split
    → min_train = 60% data
    → test_window = min(30, 10% data)

MLflow logging:
    Parent run : weighted metrics + best params + future forecast
    Child run  : per-split metrics + params (nested)
    Model      : mlflow.prophet.log_model(..., name=...) → MLflow 3.x API
"""
...
def build_splits(
    ...
    """
    Generate split indices untuk Hybrid Expanding + Sliding CV.

    Expanding (split 1-3): train_start tetap di awal, train_end gerak maju.
    Sliding   (split 4-5): train_start ikut bergeser, window size tetap.

    Fallback data pendek:
        Jika n < min_train + test_window × n_splits:
        → n_splits=3, min_train=60% data, test_window=min(30, 10% data)

    Returns list of dict per split dengan keys:
        split_idx, mode, train_start, train_end, test_start, test_end,
        n_train, n_test, date_train_start, date_train_end,
    ...
    with mlflow.start_run(
        ...
        for sp in splits:
            ...
            with mlflow.start_run(
                ...
                result = evaluate_split_prophet(model_split, dates_test, test)
                mlflow.log_metrics({
        ...
        # ── Log model — MLflow 3.x API (name= bukan artifact_path=) ──
        mlflow.prophet.log_model(
            final_model,
            name=f"Prophet_{safe_name}",
        )
